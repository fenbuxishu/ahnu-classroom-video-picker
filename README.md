...
| 总览 | 选择 | 下载记忆 |
| --- | --- | --- |
| 汇总整门课程的全部课次 | 按课次、教师相机或学生相机勾选 | 标记已下载视频并自动跳过 |
...
- **下载记忆**：再次进入课程时标记已完成视频，显示完成时间，并在批量选择时自动跳过；
- **旧记录识别**：升级后核对 Chrome 中仍存在的已完成下载，尽量补回旧版本产生的记录；
...
下载完成后，该视角会显示绿色的 **已下载** 标记。下次进入同一课程时标记仍会保留；“全选未下载”和按相机类型选择会自动跳过这些视频。如果需要重新下载某一个视频，仍可单独勾选它。
...
| `storage` | 在当前浏览器本地保存已下载标记、完成时间和必要的任务状态 |
...
- 收集或上传账号、Cookie、课程信息；
- 把视频地址发送给第三方服务；
- 保存带鉴权参数的历史视频链接（下载记录只保留移除临时参数后的地址特征）；
- 在用户未点击下载时自动下载视频；
- 注入广告、遥测或统计代码。
...
<details>
<summary><strong>如何清除“已下载”标记</strong></summary>

点击面板顶部的“清除下载标记”。它只重置当前课程的提示，不会删除已经下载到电脑里的视频。清除后，扩展不会马上从旧的 Chrome 下载记录中再次恢复这些标记；以后重新下载的视频会重新标记。
